        ...
        <ul class="nav navbar-nav">
         ...
         <li class="dropdown"><a class="dropdown-toggle" data-toggle="dropdown" href="#">Range Committee<span class="caret"></span></a>
           <ul class="dropdown-menu">
             ...
             <li class="active"><a href="portal-4-rcupdate.php">Register Committee</a></li>
...
<div class="container">
  ...
  <div class="row">
    ...
    if (isset($_POST['proceed'])) {
      ...
      $pre_query=mysqli_query($con,"SELECT * FROM members WHERE members.id='$preid'&&members.range='$range'");
      $pre=mysqli_fetch_array($pre_query);
      if (mysqli_num_rows($pre_query)==0) {
      echo "<script>
      alert('ID ($preid) does not exists or does not belongs to your range.');
      window.location.href='portal-4-rcupdate.php';
      </script>";
      }

      $sec_query=mysqli_query($con,"SELECT * FROM members WHERE members.id='$secid'&&members.range='$range'");
      $sec=mysqli_fetch_array($sec_query);
      if (mysqli_num_rows($sec_query)==0) {
      echo "<script>
      alert('ID ($secid) does not exists or does not belongs to your range.');
      window.location.href='portal-4-rcupdate.php';
      </script>";
      }

      $tres_query=mysqli_query($con,"SELECT * FROM members WHERE members.id='$tresid'&&members.range='$range'");
      $tres=mysqli_fetch_array($tres_query);
      if (mysqli_num_rows($tres_query)==0) {
      echo "<script>
      alert('ID ($tresid) does not exists or does not belongs to your range.');
      window.location.href='portal-4-rcupdate.php';
      </script>";
      }

      $vpre1_query=mysqli_query($con,"SELECT * FROM members WHERE members.id='$vpre1id'&&members.range='$range'");
      $vpre1=mysqli_fetch_array($vpre1_query);
      if (mysqli_num_rows($vpre1_query)==0) {
      echo "<script>
      alert('ID ($vpre1id) does not exists or does not belongs to your range.');
      window.location.href='portal-4-rcupdate.php';
      </script>";
      }

      $vpre2_query=mysqli_query($con,"SELECT * FROM members WHERE members.id='$vpre2id'&&members.range='$range'");
      $vpre2=mysqli_fetch_array($vpre2_query);
      if (mysqli_num_rows($vpre2_query)==0) {
      echo "<script>
      alert('ID ($vpre2id) does not exists or does not belongs to your range.');
      window.location.href='portal-4-rcupdate.php';
      </script>";
      }

      $vpre3_query=mysqli_query($con,"SELECT * FROM members WHERE members.id='$vpre3id'&&members.range='$range'");
      $vpre3=mysqli_fetch_array($vpre3_query);
      if (mysqli_num_rows($vpre3_query)==0) {
      echo "<script>
      alert('ID ($vpre3id) does not exists or does not belongs to your range.');
      window.location.href='portal-4-rcupdate.php';
      </script>";
      }

      $jsec1_query=mysqli_query($con,"SELECT * FROM members WHERE members.id='$jsec1id'&&members.range='$range'");
      $jsec1=mysqli_fetch_array($jsec1_query);
      if (mysqli_num_rows($jsec1_query)==0) {
      echo "<script>
      alert('ID ($jsec1id) does not exists or does not belongs to your range.');
      window.location.href='portal-4-rcupdate.php';
      </script>";
      }

      $jsec2_query=mysqli_query($con,"SELECT * FROM members WHERE members.id='$jsec2id'&&members.range='$range'");
      $jsec2=mysqli_fetch_array($jsec2_query);
      if (mysqli_num_rows($jsec2_query)==0) {
      echo "<script>
      alert('ID ($jsec2id) does not exists or does not belongs to your range.');
      window.location.href='portal-4-rcupdate.php';
      </script>";
      }

      $jsec3_query=mysqli_query($con,"SELECT * FROM members WHERE members.id='$jsec3id'&&members.range='$range'");
      $jsec3=mysqli_fetch_array($jsec3_query);
      if (mysqli_num_rows($jsec3_query)==0) {
      echo "<script>
      alert('ID ($jsec3id) does not exists or does not belongs to your range.');
      window.location.href='portal-4-rcupdate.php';
      window.
      </script>";
      }

      $coun_query=mysqli_query($con,"SELECT * FROM members WHERE members.id='$counid'&&members.range='$range'");
      $coun=mysqli_fetch_array($coun_query);
    if (mysqli_num_rows($coun_query)==0) {
    echo "<script>
    alert('ID ($counid) does not exists or does not belongs to your range.');
    window.location.href='portal-4-rcupdate.php';
    </script>";
    }
    }
    ...
    <div class="col-*-*">
      <div class="alert alert-danger">
        ...
        <div class="panel panel-default">
          <div class="panel-heading">Range Committee Registration</div>
              ...
              <div class="alert alert-info" name=vpre>
                ...
                <div class="alert alert-warning" name=vpre3>
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-10">
                      <input type="text" id=input class="form-control" value="<?php echo $vpre3['id'];?>" name="vpre3id" readonly>
                    </div>
                  </div>
                  <div class="form-group">
                    <label class="control-label col-sm-2" for="vpre3name">Name:</label>
                    <div class="col-sm-10">
                      <input type="text" id=input class="form-control" value="<?php echo $vpre3['name'];?>" name="vpre3name" readonly>
                    </div>
                  </div>
                  <div class="form-group">
                    <label class="control-label col-sm-2" for="vpre3mob">Mobile:</label>
                    <div class="col-sm-10">
                      <input type="text" class="form-control" value="<?php echo $vpre3['mob'];?>" name="vpre3mob" readonly>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="alert alert-info" name=jsec>
                ...
                <div class="alert alert-warning" name=jsec2>
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-10">
                      <input type="text" class="form-control" value="<?php echo $jsec3['mob'];?>" name="jsec2mob" readonly>
                    </div>
                  </div>
                </div>
                ...
                <div class="alert alert-warning" name=jsec3>
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-10">
                      <input type="text" id=input class="form-control" value="<?php echo $jsec3['id'];?>" name="jsec3id" readonly>
                    </div>
                  </div>
                  <div class="form-group">
                    <label class="control-label col-sm-2" for="jsec3name">Name:</label>
                    <div class="col-sm-10">
                      <input type="text" id=input class="form-control" value="<?php echo $jsec3['name'];?>" name="jsec3name" readonly>
                    </div>
                  </div>
                  <div class="form-group">
                    <label class="control-label col-sm-2" for="jsec3mob">Mobile:</label>
                    <div class="col-sm-10">
                      <input type="text" class="form-control" value="<?php echo $jsec3['mob'];?>" name="jsec3mob" readonly>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="form-group" name=button_validation>
                <center>
                  ...
                  if (isset($_POST['register'])) {
                    ...
                    $vpre3id=$_POST['vpre3id'];
                    $vpre3name=$_POST['vpre3name'];
                    $vpre3mob=$_POST['vpre3mob'];
                    $jsec1id=$_POST['jsec1id'];
                    ...
                    $jsec3id=$_POST['jsec3id'];
                    $jsec3name=$_POST['jsec3name'];
                    $jsec3mob=$_POST['jsec3mob'];
                    $counid=$_POST['counid'];
                    $counname=$_POST['counname'];
                    $counmob=$_POST['counmob'];

                    /*Zone Committee registration*/
                    $sq="INSERT INTO rcdetails VALUES ('$zone','$range','$preid',
                      '$prename','$preads','$premob',
                      '$secid','$secname','$secads','$secmob','$tresid','$tresname','$tresads',
                      '$tresmob','$vpre1id','$vpre1name','$vpre1mob',
                      '$vpre2id','$vpre2name','$vpre2mob','$vpre3id','$vpre3name','$vpre3mob',
                      '$jsec1id','$jsec1name','$jsec1mob','$jsec2id','$jsec2name','$jsec2mob','$jsec3id',
                      '$jsec3name','$jsec3mob','$counid','$counname','$counmob')";
                      if ($con->query($sq) == TRUE) {
                        echo "<script>
                        alert('$range Range Committee Successfully Registered !');
                        window.location.href='portal-2-rcoverview.php';
                        </script>";
                      }if ($con->query($sq) == FALSE) {
                        echo "<script>
                        alert('$range Range Committee already Registered !');
